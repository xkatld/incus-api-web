...
app = Flask(__name__)
app.secret_key = '请在这里替换成一个随机的、安全的字符串'
...
def add_nat_rule_logic(name, host_port, container_port, protocol):
    ...
    if success_iptables:
        ...
        if not db_success:
             ...
             return True, message, {'rule_id': None}, 200
...
def delete_nat_rule_logic(rule_id):
    ...
    if not rule:
        ...
        return False, f'数据库中找不到ID为 {rule_id} 的NAT规则记录，可能已被手动删除。跳过 iptables 删除。', 404
...
def index():
    ...
    images_list, image_error = get_images_list_data()
    if incus_error[0]:
...
@app.route('/container/nat_rule/<int:rule_id>', methods=['POST'])
