...
def create_container():
    ...
    if not name or not image: return jsonify({'status': 'error', 'message': '容器名称和镜像不能为空'}), 400
    if query_db('SELECT 1 FROM containers WHERE incus_name = ?', [name], one=True): return jsonify({'status': 'error', 'message': f'名称为 "{name}" 的容器已存在。'}), 409
    
    command = ['launch', image, name]
    if data.get('storage_pool'): command.extend(['-s', data['storage_pool']])
    if data.get('cpu_cores'): command.extend(['-c', f'limits.cpu={data["cpu_cores"]}'])
    if data.get('cpu_allowance'): command.extend(['-c', f'limits.cpu.allowance={data["cpu_allowance"]}%'])
    if data.get('memory_mb'): command.extend(['-c', f'limits.memory={data["memory_mb"]}MB'])
    if data.get('disk_gb'): command.extend(['-d', f'root,size={data["disk_gb"]}GB'])
    if data.get('security_nesting') == 'on': command.extend(['-c', 'security.nesting=true'])
    ...
    if success:
        ...
        if list_output: sync_container_to_db(name, list_output[0].get('config',{}).get('image.description', image), list_output[0].get('status', 'Pending'), list_output[0].get('created_at'))
...
def container_action(name):
    ...
    if action == 'delete':
        ...
        return jsonify({'status': 'error', 'message': f'删除容器 {name} 失败: {output}'}), 500

    commands = {'start': ['start', name], 'stop': ['stop', name, '--force'], 'restart': ['restart', name, '--force']}
    if action not in commands: return jsonify({'status': 'error', 'message': '无效的操作'}), 400
    
    success, output = incus_commands.run_incus_command(commands[action], parse_json=False)
    if success:
        time.sleep(2)
        _, list_output = incus_commands.run_incus_command(['list', name, '--format', 'json'])
        if list_output: sync_container_to_db(name, list_output[0].get('config', {}).get('image.description', 'N/A'), list_output[0].get('status', '未知'), list_output[0].get('created_at'))
...
def exec_command_in_container(name):
    ...
    if not command_to_exec: return jsonify({'status': 'error', 'message': '命令不能为空'}), 400
    ...
    return jsonify({'status': 'success' if success else 'error', 'output': output}), 200 if success else 500
...
def add_nat_rule_route(name):
    ...
    info, error = incus_commands.get_container_raw_info(name)
    if not info or info.get('status') != 'Running' or not info.get('ip') or info.get('ip') == 'N/A': return jsonify({'status': 'error', 'message': '无法获取容器IP或容器未运行'}), 400
...
def delete_nat_rule(rule_id):
    ...
    success_ipt, _, is_bad = nat_manager.perform_iptables_delete_for_rule(rule)
    if success_ipt or is_bad:
        remove_nat_rule_from_db(rule_id)
        return jsonify({'status': 'success', 'message': 'NAT 规则已删除。'})
    return jsonify({'status': 'error', 'message': '删除 NAT 规则失败'}), 500
...
def add_quick_command_route():
    ...
    if success: return jsonify({'status': 'success', 'id': result})
...
def add_reverse_proxy_route(name):
    ...
    if not info or info.get('status') != 'Running' or not info.get('ip') or info.get('ip') == 'N/A': return jsonify({'status': 'error', 'message': '容器未运行或无IP'}), 400
...
def delete_reverse_proxy_rule(rule_id):
    ...
    if not rule: return jsonify({'status': 'warning', 'message': '规则记录未找到'}), 200
    success_nginx, _ = nginx_manager.delete_reverse_proxy(rule['domain'])
    if success_nginx:
        remove_reverse_proxy_rule_from_db(rule_id)
        return jsonify({'status': 'success', 'message': '反向代理规则已删除。'})
    return jsonify({'status': 'error', 'message': '删除反向代理失败'}), 500
